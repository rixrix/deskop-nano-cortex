...
- **`release.yml`** — Installer factory. Triggered by manual `workflow_dispatch` with a `platform`
  ...
  Tauri native bundles on each, and attaches all artifacts to a drafted GitHub Release. Signing
  secrets are consumed when present and skipped gracefully when absent for v1.0.0.
    ...
    steps:
      ...
      - name: Build Tauri app + bundle
        ...
        env:
          ...
          # macOS signing — skipped when absent (v1.0.0)
          APPLE_CERTIFICATE: ${{ secrets.APPLE_CERT_P12_BASE64 }}
          APPLE_CERTIFICATE_PASSWORD: ${{ secrets.APPLE_CERT_PASSWORD }}
          APPLE_SIGNING_IDENTITY: ${{ secrets.APPLE_SIGNING_IDENTITY }}
          APPLE_ID: ${{ secrets.APPLE_API_KEY_ID }}
          APPLE_TEAM_ID: ${{ secrets.APPLE_API_ISSUER_ID }}
          APPLE_API_KEY: ${{ secrets.APPLE_API_KEY_P8 }}
          # Windows signing — skipped when absent (v1.0.0)
...
No signing secrets are configured. Artifacts are unsigned. Platform OS warnings appear:
...
When `APPLE_CERT_P12_BASE64` and related secrets are present:
...
| v1.0.0 unsigned                               | Skip signing until certs are available         | User has no Developer ID (resolved in archived sprint brief: Q2). Signing infra (entitlements, secret schema, workflow env vars) is in place from day one so a signed release is config-only. |
