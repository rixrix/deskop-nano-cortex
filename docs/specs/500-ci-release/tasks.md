...
- [x] Add `backend` job: `dtolnay/rust-toolchain@stable` (components: `rustfmt`, `clippy`), `Swatinem/rust-cache@v2` (workspaces: `backend`), Linux system deps (incl. `libdbus-1-dev` for BLE), `cargo fmt --check`, `cargo clippy --features ble --all-targets -- -D warnings`, `cargo test --features ble`.
...
- [x] Add per-leg steps: checkout, `setup-node@v4`, `dtolnay/rust-toolchain@stable`, `Swatinem/rust-cache@v2`, Linux system deps (conditional on `ubuntu-latest`), `npm ci`.
- [x] Add `tauri-apps/tauri-action@v0` step with `projectPath: backend`, `releaseDraft: true`, `tagName: ${{ github.ref_name }}`, and the signing env var block (secrets passed through; skipped gracefully when absent).
...
<!-- @see docs/specs/500-ci-release/spec.md [FR-13] [FR-14] [FR-15] [FR-16] [FR-17] [FR-18] -->
