...
> Release. Code signing is deferred to a later signed release; v1.0.0 ships unsigned with
> documented Gatekeeper and SmartScreen caveats. Windows and Linux artifacts are build-wired but
> remain untested previews until platform smoke evidence is recorded.
...
2. Produces installable artifacts for the selected platform(s) — `dmg`/`app`, `msi`/`exe`, and
   ...
   GitHub Release.
3. Documents the code-signing secret schema so that a later signed release is configuration-only:
   drop in secrets, nothing else changes.
...
| FR-8  | The release workflow documents and consumes the signing secret schema (see table below) when secrets are present; signing is skipped gracefully when secrets are absent (v1.0.0).                                                                                                                                                                                      | Must Have   |
...
- [ ] `release.yml` runs without any signing secrets in v1.0.0 (unsigned artifacts).
- [ ] README documents the Gatekeeper and SmartScreen workarounds for unsigned v1.0.0 artifacts.
- [ ] Adding `APPLE_CERT_P12_BASE64` and related secrets (see table below) enables macOS signing without workflow edits.
- [ ] Adding `WINDOWS_CERT_PFX_BASE64` and `WINDOWS_CERT_PASSWORD` enables Windows signing without workflow edits.
...
These secrets are consumed by `release.yml`. None are required for `ci.yml`. In v1.0.0 all signing
steps are skipped (secrets absent). Adding a secret later enables the corresponding signing path
without any workflow source edit.
...
> **v1.0.0 note**: All signing secrets are absent. Artifacts are unsigned. macOS users must
> right-click → Open to bypass Gatekeeper. Windows users must click "More info → Run anyway"
> to bypass SmartScreen. This is documented in the project README.
...
| `.github/workflows/release.yml`  | [FR-3] [FR-4] [FR-5] [FR-6] [FR-8] [FR-9] [FR-11] [FR-14] [FR-18] | Manual platform-targeted cross-platform build + GH Release draft                                                   | Artifact smoke   | Notarization stapling, updater                              |
